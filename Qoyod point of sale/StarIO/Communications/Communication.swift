...
class Communication {
    static func sendCommands(_ commands: Data!, port: SMPort!, completionHandler: SendCompletionHandler?) -> Bool {
        ...
        while true {
            if port == nil {
                title   = languageBundle!.localizedString(forKey: "Fail to Open Port", value: "", table: nil)
                message = ""
                break
            }
            ...
            if printerStatus.offline == sm_true {
                title   = languageBundle!.localizedString(forKey: "Printer Error", value: "", table: nil)
                message = languageBundle!.localizedString(forKey: "Printer is offline (BeginCheckedBlock)", value: "", table: nil)
                break
            }
            ...
            while total < UInt32(commands.count) {
                ...
                if Date().timeIntervalSince(startDate) >= 30.0 {     // 30000mS!!!
                    title   = languageBundle!.localizedString(forKey: "Printer Error", value: "", table: nil)
                    message = languageBundle!.localizedString(forKey: "Write port timed out", value: "", table: nil)
                    break
                }
            }
            ...
            if printerStatus.offline == sm_true {
                title   = languageBundle!.localizedString(forKey: "Printer Error", value: "", table: nil)
                message = languageBundle!.localizedString(forKey: "Printer is offline (EndCheckedBlock)", value: "", table: nil)
                break
            }
            
            title   = languageBundle!.localizedString(forKey: "Send Commands", value: "", table: nil)
            message = languageBundle!.localizedString(forKey: "Success", value: "", table: nil)
