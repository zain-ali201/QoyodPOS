...
open class BarcodeScannerViewController: UIViewController {
   ...
   */
  public func resetWithError(flag: Bool)
  {
    if flag
    {
   ...
   */
  public func reset(flag: Bool)
  {
    if flag
    {
        status = Status(state: .scanning, animated: true)
    }
    else
    {
        status = Status(state: .notFound, text: "No product found.")
    }
  }
